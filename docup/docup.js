...
const Sidebar = ({menu, title, base, showSidebar, navLinks}) => {
  const [hash, setHash] = l("");
  const sidebarRef = s(null);
  ...
  y(() => {
    ...
    const onHashChange = () => {
      ...
      if (location.hash && !sidebarItemClicked) {
        const el = document.querySelector(`.sidebar .menu_item[href="${decodedHash}"]`);
        if (el) {
  ...
  return /* @__PURE__ */ v$1("div", {
    ref: sidebarRef,
    class: `fixed md:pt-12 sidebar left-0 top-0 bottom-0` + (showSidebar ? " sidebar_show" : "")
  }, /* @__PURE__ */ v$1("div", {
    ...
    class: "w-full",
    href: base
  }, title)), /* @__PURE__ */ v$1("div", null, /* @__PURE__ */ v$1("ul", null, navLinks.map((link, index) => {
    return /* @__PURE__ */ v$1("li", {
      key: index
    }, /* @__PURE__ */ v$1("a", {
      class: "navlink",
    ...
    return /* @__PURE__ */ v$1("a", __assign$2({
      class: `block px-5 py-1 menu_item` + (item.slug === hash.slice(1) ? " menu_item__active" : ""),
...
const Main = ({html, loadingState}) => {
  ...
  }, loadingState === "loading" ? /* @__PURE__ */ v$1("section", {
    class: ""
  }, /* @__PURE__ */ v$1("div", {
...
async function loadLanguages(langs) {
  await Promise.all(langs.map((lang) => loadScript(`https://cdn.jsdelivr.net/npm/prismjs@${"^1.23.0"}/components/prism-${lang}.min.js`)));
}
  ...
  return function() {
    ...
    if (!timeoutID)
      if (delta >= wait)
        ...
        timeoutID = self.setTimeout(call, wait - delta);
    return rtn;
  };
  function call() {
    timeoutID = 0;
...
const initMarkdownComponentsProxy = () => {
  window._MD_COMPONENTS = new Proxy({}, {
    set: function(target, prop, value) {
      ...
      const importRenderFunction = info === "react" ? `import {createElement as h} from "${getSkypackUrl("react")}";
              import {render} from "${getSkypackUrl("react-dom")}"` : info === "vue" ? `import {createApp,h} from '${getSkypackUrl("vue@next")}';var render=(app,el)=>createApp(app).mount(el)` : `import {render,h} from "${docupUrl}"`;
...
function renderMarkdown(text) {
  ...
  renderer.blockquote = (quote) => {
    ...
    if (m) {
      const [, type] = m;
      return `<div class="message message_type__${type.toLowerCase()}"><p>${quote.replace(BLOCKQUOTE_TAG_RE, "")}</div>`;
    }
  ...
  renderer.code = (code, _lang = "", escaped) => {
    ...
    if (info === "preact" || info === "fre" || info === "react" || info === "vue") {
      const index = codeReplacementIndex++;
      fns.push(() => {
        const componentScript = document.createElement("script");
        componentScript.type = "module";
        componentScript.textContent = `
        ${rewriteImports(code).replace(/\bexport default\b/, `window._MD_COMPONENTS.${info}_${index}=`)}`;
        setTimeout(() => document.body.append(componentScript));
      });
      return `${keep === "above" ? renderCode() : ""}<div id="md_components_${index}"></div>${keep === true ? renderCode() : ""}`;
    }
    return renderCode();
  };
...
const handleScroll = throttle(() => {
  ...
  for (let i = 0; i < headings.length; i++) {
    ...
    if (elementInView_common(heading, {
      offset: 58
    })) {
      const id = heading.id;
      if (id) {
...
const App = ({options}) => {
  ...
  const [html, setHtml] = l("");
  const [loadingState, setLoadingState] = l("loading");
  const [menu, setMenu] = l([]);
  const [showSidebar, setShowSidebar] = l(false);
  ...
  return /* @__PURE__ */ v$1("div", null, showSidebar && /* @__PURE__ */ v$1("div", {
...
class Docup {
  ...
  init() {
    ...
    S(v$1(App, {options: this.options}), document.body);
  }
}
...
const version = "1.0.0";
const html = htm.bind(v$1);

export { Docup, v$1 as h, html, init, S as render, setHtml, A as useCallback, F as useContext, T as useDebugValue, y as useEffect, q as useErrorBoundary, _ as useImperativeHandle, h as useLayoutEffect, d as useMemo, p as useReducer, s as useRef, l as useState, version };
